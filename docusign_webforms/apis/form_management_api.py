...
class FormManagementApi(object):
    ...
    def get_form(self, account_id, form_id, **kwargs):
        ...
        :param str state: The state of the form content
        :return: WebForm
    ...
    def list_forms_with_http_info(self, account_id, **kwargs):
        ...
        all_params = ['account_id', 'user_filter', 'is_standalone', 'is_published', 'state', 'sort_by', 'search', 'start_position', 'count']
        all_params.append('callback')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        if 'state' in params and len(params['state']) > 15:
            raise ValueError("Invalid value for parameter `state` when calling `list_forms`, length must be less than or equal to `15`")
